...
- AUTOMATIC inference of readout, spokes, and coils from headers/FID (no CLI RO/spokes needed)
- Handles blocked readout padding (e.g., true RO=420 stored in 512)
- Default: reads trajectory from file (`traj.cfl/.hdr` or `traj.npy`), optional golden-angle generator
...
# =========================================================
DEBUG = False
def dbg(*args):  # simple gated print
    if DEBUG:
        print("[debug]", *args)
...
def load_bruker_kspace(series_dir: Path,
    ...
    # Readout from headers (true RO, not stored block)
    if readout is None:
        acq_size = _parse_acq_size(method, acqp)
        if acq_size:
            readout = acq_size[0]
    dbg("readout (header):", readout)

    # Collapse extras & coils to per-coil bucket
    denom = coils * max(1, other_dims)
    if total_cplx % denom != 0:
        # Try relaxing other_dims if headers lied
        dbg("total not divisible by coils*other_dims; relaxing other_dims->1")
        denom = coils
        if total_cplx % denom != 0:
            # try also relaxing coils to 1 (worst case)
    ...
    # Determine stored_ro (block) and spokes
    def pick_block_and_spokes(per_coil_total: int, readout_hint: Optional[int], spokes_hint: Optional[int]) -> Tuple[int, int]:
        ...
        # Candidate block sizes (prefer >= readout)
        ...
        if readout_hint:
            cands = [b for b in BLOCKS if b >= readout_hint] + [per_coil_total]  # allow exact fit
        else:
            cands = BLOCKS + [per_coil_total]
        for b in cands:
            if b > 0 and per_coil_total % b == 0:
                return b, per_coil_total // b
        # Fallback: keep RO if it divides
        if readout_hint and per_coil_total % readout_hint == 0:
            return readout_hint, per_coil_total // readout_hint
        # Last fallback: prime factorization-ish guess (use nearest divisor around readout or sqrt)
    ...
    # Rebuild total spokes including extras we collapsed
    ...
    if readout is not None and stored_ro >= readout:
        ...
        ksp = ksp_blk
        if readout is None:
            # If no header RO, set to stored_ro so caller knows dims
            readout = stored_ro
...
def main():
    ...
    ap.add_argument('--traj-file', type=Path, help='traj.cfl/.hdr or traj.npy (shape (3,RO,Spokes))')
    ...
    # Load k-space (auto infers dims; can be overridden via CLI)
    ...
    ksp_base = args.out.with_name(args.out.name + '_ksp')
    write_cfl(ksp_base, ksp)

    # Trajectory (3, ro, sp)
    if args.traj == 'golden':
        traj = golden_angle_3d(TrajSpec(readout=ro, spokes=sp, matrix=tuple(args.matrix)))
    else:
    ...
    traj_base = args.out.with_name(args.out.name + '_traj')
    ...
    if dcf_mode.startswith('pipe'):
        ...
        dcf = dcf_pipe(traj, iters=nit, grid_shape=tuple(args.matrix), gpu=args.gpu)
        dcf_base = args.out.with_name(args.out.name + '_dcf')
        write_cfl(dcf_base, dcf.astype(np.complex64))
        kspw_base = args.out.with_name(args.out.name + '_kspw')
    ...
    # Recon
    if args.iterative:
        out_img = args.out.with_name(args.out.name + '_recon')
        recon_iterative(traj_base, ksp_in, out_img, lam=args.lam, iters=args.iters, wavelets=args.wavelets, gpu=args.gpu)
    else:
        out_img = args.out.with_name(args.out.name + '_adj')
        recon_adjoint(traj_base, ksp_in, args.combine, out_img, gpu=args.gpu)

    # Export
    if args.export_nifti:
        run_bart(['toimg', str(out_img), str(args.out)], gpu=args.gpu)
        print(f"[info] Wrote NIfTI: {args.out}.nii")
    else:
        print(f"[info] Recon complete. BART base: {out_img} (.cfl/.hdr)")
