...
- Bruker FID loader: infers RO/Spokes/Coils and trims blocked-RO padding.
- Trajectory: if $series/traj exists, it is ALWAYS used (no silent fallback).
  * Accepts binary float32/float64 or ASCII, any of: flat 3*RO*SP, (RO*SP,3), (3,RO*SP).
  * If size != 3*RO*SP, it trims or zero-pads to match (with warnings).
- DCF: Pipe-style (NumPy), no BART calls.
- Recon:
  * Primary: BART adjoint NUFFT (+ SoS/SENSE) with sticky CPU fallback.
  * Fallback: Pure NumPy adjoint gridding (Kaiser–Bessel) via --force-python-adjoint or on BART failure.
        ...
        for src in srcs:
            if key in src:
                try:
                    txt = src[key]; num=""; tmp=""
                    for ch in txt:
                        if ch.isdigit() or (ch in "+-" and not tmp): tmp+=ch
                        elif tmp: num=tmp; break
                    if not num and tmp: num=tmp
...
def _read_bruker_traj_strict(series_dir: Path, ro: int, sp: int) -> np.ndarray:
    ...
    for dt in (np.float32, np.float64, None):  # None -> ASCII
        ...
        # try reshaping table forms first
        if nsamp == ro*sp:
            return vals.reshape(3, ro, sp, order="F")
        # if table (ro*sp,3)
        if nsamp == ro*sp:
        ...
        if nsamp > need:
            ...
            pad = np.zeros(3*(need-nsamp), dtype=np.float32)
...
def recon_iterative(traj_base: Path, ksp_base: Path, out_base: Path, lam: float, iters: int, wavelets: Optional[int], gpu: bool):
    ...
    if wavelets is not None: cmd+=["-R",f"W:7:{wavelets}:{lam}"]
    elif lam>0: cmd+=["-R",f"W:7:0:{lam}"]
...
def main():
    ...
    # save ksp in non-Cart layout for BART paths
    ...
    # trajectory: STRICT use of $series/traj if present
    traj_path = series_dir/"traj"
    if traj_path.exists():
        traj = _read_bruker_traj_strict(series_dir, ro, sp)
    else:
        # only if not present, consider external or golden
    ...
    if dcf_mode.startswith("pipe"):
        ...
        dcf16 = dcf.astype(np.complex64).reshape(ro,sp,*([1]*14))
        perm=[2,3,4,5,6,7,8,9,12,13,0,1,10,11,14,15]
        dcf16 = np.transpose(dcf16, perm)
        dims=[1]*16; dims[10]=ro; dims[11]=sp
        _write_cfl(dcf_base, dcf16, dims)
        # apply DCF to ksp (NumPy) for BART/py paths
        ksp_arr = read_cfl(ksp_base)
        dcf_b = _make_weight_like(ksp_arr, dcf)
        kspw = ksp_arr*dcf_b
        kspw_base = out_base.with_name(out_base.name+"_kspw")
        _write_cfl(kspw_base, kspw, list(ksp_arr.shape)+[1]*(16-len(ksp_arr.shape)))
        ksp_in = kspw_base
        ksp_for_python = ksp * dcf[..., None]  # <<< fixed broadcast
    else:
        ksp_in = ksp_base
        ksp_for_python = ksp
    ...
    if args.iterative:
        ...
        if args.force_python_adjoint:
            recon_adjoint_python(traj, ksp_for_python, dcf, (NX,NY,NZ), out_img, combine=args.combine)
        else:
            try:
