...
def readcfl(name: str) -> np.ndarray:
    ...
    if cplx.size != expected:
        raise ValueError(f"CFL size mismatch: have {cplx.size} complex, expected {expected} from dims {dims} for {base}")
...
def run_bart(
    ...
    if grad_path.exists():
        ...
        traj_full = build_traj_from_dirs(true_ro, dirs_full, NX, traj_scale, readout_origin, reverse_readout)
    else:
        raise RuntimeError(f"grad.output not found in {series_path} (this run requires it).")
