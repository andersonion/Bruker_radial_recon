...
# ---------- Kronecker / linear-Z GA trajectory ----------
...
def build_synthetic_traj(mode: str, ro: int, sp_total: int) -> np.ndarray:
    """
    mode: 'kron' or 'linz'
    returns traj with shape (3, RO, Spokes) in normalized k-space units (~[-0.5,0.5]).
    """
    mode = mode.lower()
    N = sp_total
...
def ksp_to_bart_noncart(ksp: np.ndarray) -> Tuple[np.ndarray, List[int]]:
    ...
    # dims: [1,1,1,COILS,1,1,1,1,1,1,RO,SP,1,1,1,1]
    dims = [1]*16
    dims[3] = nc
    dims[10] = ro
    dims[11] = sp
...
def bart_recon_frame(traj: np.ndarray, ksp: np.ndarray, out_base: Path, combine: str, matrix: Tuple[int,int,int]):
    NX, NY, NZ = matrix
    ro, sp, nc = ksp.shape
    ...
    if combine.lower() == "sos":
        ...
        raise ValueError("Only sos combine currently supported in this script")

def cfl_to_3d_numpy(base: Path, NX: int, NY: int, NZ: int) -> np.ndarray:
    arr = read_cfl(base)
    arr = np.squeeze(arr)
    if arr.ndim < 3:
        raise ValueError(f"CFL image has ndim={arr.ndim}, cannot form 3D volume")
    if arr.shape[0] < NX or arr.shape[1] < NY or arr.shape[2] < NZ:
        raise ValueError(f"CFL image too small {arr.shape} for requested ({NX},{NY},{NZ})")
...
def main():
    global DEBUG
    ap = argparse.ArgumentParser(description="Bruker 3D radial recon using BART (sliding window, GA/Kron traj)")
    ...
    ap.add_argument("--combine", type=str, default="sos")
    ap.add_argument("--traj-mode", type=str, choices=["kron","linz","linear_z"], default="kron",
    ...
    if not bart_exists():
        ...
        sys.exit(1)
    if nib is None:
        print("WARNING: nibabel not available; NIfTI writing will fail.", file=sys.stderr)
    ...
    if traj_path.with_suffix(".cfl").exists() and traj_path.with_suffix(".hdr").exists():
        ...
        # raw binary (float32/float64) or ascii
        data = np.fromfile(traj_path, dtype=np.float32)
        if data.size != 3*ro*sp_total:
            print(f"[warn] traj file size {data.size} != 3*RO*Sp={3*ro*sp_total}; trying float64...")
            data = np.fromfile(traj_path, dtype=np.float64)
        if data.size != 3*ro*sp_total:
            raise ValueError("traj file size mismatch; expected 3*RO*Sp samples.")
        traj = data.astype(np.float32).reshape(3, ro, sp_total, order="F")
    else:
        mode = "kron" if args.traj_mode in ("kron",) else "linz"
    ...
    if traj.shape != (3, ro, sp_total):
        raise ValueError(f"Final trajectory shape {traj.shape} != (3,{ro},{sp_total})")
    ...
    if spf is None:
        spf = min(sp_total, 1000)
        print(f"[warn] --spokes-per-frame not set; defaulting to {spf}")
    ...
    if not starts:
        raise ValueError("No frames to reconstruct with current spokes-per-frame / frame-shift.")
    ...
    for fi, s0 in enumerate(starts, 1):
        ...
        frames.append(img3)
        print(f"[info] Frame {fi}/{len(starts)} done")
    ...
    # QC volume
    qc_idx = max(1, min(args.qc_frame, vol4d.shape[3])) - 1
    qc_img = vol4d[..., qc_idx]
    ...
    if nib is not None:
        ...
        print(f"[info] Wrote QC NIfTI: {qc_out}")
    else:
        print(f"[warn] nibabel not available; skipping QC NIfTI {qc_out}")
    ...
    if nib is not None:
        ...
        print(f"[info] Wrote 4D NIfTI: {out_nii}")
    else:
        print(f"[warn] nibabel not available; skipping 4D NIfTI {out_nii}")
