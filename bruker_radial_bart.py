...
def infer_ro_and_block_from_acqp(acqp: Dict[str, str]) -> Tuple[Optional[int], Optional[int]]:
    """
    From ACQ_size, infer:
      - true readout RO (first entry)
      - stored block length (second entry), which Bruker often pads to on disk
    Example:
        ##$ACQ_size=( 3 )
        122 2584 1
      -> RO_true = 122, stored_block = 2584
    """
    val = acqp.get("ACQ_size", "")
    ints = _parse_int_list(val)
    ro_true = ints[0] if len(ints) >= 1 else None
    stored_block = ints[1] if len(ints) >= 2 else None
    return ro_true, stored_block


def load_bruker_kspace(
    series_dir: Path,
    coils: int,
    ro_true: Optional[int] = None,
    stored_ro_hint: Optional[int] = None,
    fid_dtype: Optional[str] = None,
    fid_endian: Optional[str] = None,
) -> np.ndarray:
    """
    Load Bruker FID and reshape into k-space as (RO_true, Spokes, Coils),
    accounting for Bruker storing each spoke in a larger padded block
    (stored_ro >= RO_true).

    Strategy:
      1. Read FID as complex array.
      2. total = cpx.size
      3. per_coil_total = total / coils
      4. If stored_ro_hint is provided and divides per_coil_total,
         use stored_ro = stored_ro_hint; spokes = per_coil_total / stored_ro.
         Otherwise, fall back to searching a factor near ro_true.
      5. Reshape (stored_ro, spokes, coils) and trim to RO_true
         along the readout dimension.
    """
    ...
    if total % coils != 0:
        raise ValueError(f"FID length {total} not divisible by coils={coils}.")
    per_coil_total = total // coils

    # Decide stored_ro (block length on disk) and spokes from per_coil_total
    stored_ro = None
    spokes = None

    # Prefer the ACQ_size second entry as stored_ro if it fits
    if stored_ro_hint is not None and stored_ro_hint > 0 and per_coil_total % stored_ro_hint == 0:
        stored_ro = stored_ro_hint
        spokes = per_coil_total // stored_ro
        dbg("Using stored_ro from ACQ_size[1]:", stored_ro, "spokes:", spokes)

    # If no good stored_ro yet, fall back to using ro_true directly if that divides
    if stored_ro is None and ro_true is not None and ro_true > 0 and per_coil_total % ro_true == 0:
        stored_ro = ro_true
        spokes = per_coil_total // stored_ro
        dbg("Using stored_ro == ro_true:", stored_ro, "spokes:", spokes)

    # Last resort: search for a factor near ro_true
    if stored_ro is None:
        if ro_true is None or ro_true <= 0:
            raise ValueError(
                "Cannot determine stored_ro: ro_true is unknown and ACQ_size[1] "
                "didn't produce a valid factorization."
            )
        best = None
        max_search = min(per_coil_total, ro_true * 100)
        for cand in range(ro_true, max_search + 1):
            if per_coil_total % cand == 0:
                best = cand
                break
        if best is None:
            raise ValueError(
                f"Could not factor per_coil_total={per_coil_total} into stored_ro * spokes "
                f"with stored_ro >= ro_true={ro_true}."
            )
        stored_ro = best
        spokes = per_coil_total // stored_ro
        dbg("Using searched stored_ro:", stored_ro, "spokes:", spokes)

    if stored_ro is None or spokes is None:
        raise ValueError("Internal error: stored_ro/spokes not set after factoring.")

    # Now reshape to (stored_ro, spokes, coils)
    ksp_blk = np.reshape(cpx, (stored_ro, spokes, coils), order="F")

    # Trim down to true RO if known and stored_ro >= ro_true
    if ro_true is not None and 0 < ro_true <= stored_ro:
        ksp = ksp_blk[:ro_true, :, :]
        ro_used = ro_true
        print(
        ...
        ro_used = stored_ro
        print(
            f"[info] Loaded k-space using stored_ro={stored_ro} as RO (no smaller true RO available); "
            f"Spokes={spokes}, Coils={coils}"
        )
...
def main():
    ...
    # RO true + stored block from ACQ_size, unless overridden
    ro_true, stored_block = infer_ro_and_block_from_acqp(acqp)
    if args.readout is not None:
        ro_true = args.readout
        stored_block = None  # let loader search if needed
    if ro_true is None:
        raise ValueError("Could not infer true readout (RO) from ACQ_size; please pass --readout.")
    if stored_block is not None:
        print(f"[info] Readout (true RO) from ACQ_size: RO={ro_true}, stored block = {stored_block}")
    else:
        print(f"[info] Readout (true RO) from ACQ_size: RO={ro_true} (no stored block hint)")
    ...
    # k-space (handles padded blocks and trims to ro_true)
    ksp = load_bruker_kspace(
        series_dir,
        coils=coils,
        ro_true=ro_true,
        stored_ro_hint=stored_block,
